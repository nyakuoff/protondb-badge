...
function _findInDoc(doc: Document): HTMLElement | null {
  ...
  const svgs = doc.querySelectorAll(
    '[class*="SVGIcon_PlayTime"], .SVGIcon_PlayTime'
  );
  if (svgs.length > 0) {
    const row =
      svgs[svgs.length - 1].parentElement?.parentElement?.parentElement;
...
export function findToolbarRow(doc: Document): ToolbarTarget | null {
  ...
  for (const popup of popups) {
    ...
    if (found) {
      console.log(
        '[ProtonDB] findToolbarRow: found in popup',
        popup?.m_strName ?? popup?.m_strTitle ?? '?'
      );
  ...
  console.log(
    '[ProtonDB] findToolbarRow: NOT FOUND | PlayBar.GameStatsSection:',
    PlayBar?.GameStatsSection,
    '| popups checked:',
    checked,
    '| total popups:',
    popups.length
